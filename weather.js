    ...
    for (const city of cities) {
        try {
            ...
            if (row) {
                ...
                const pollResponse = await fetch(`https://api.openweathermap.org/data/2.5/air_pollution?lat=${data.coord.lat}&lon=${data.coord.lon}&appid=${apiKey}`);
                ...
                row.querySelector(".sunrise").innerHTML = new Date(data.sys.sunrise * 1000).toLocaleTimeString();
                row.querySelector(".sunset").innerHTML = new Date(data.sys.sunset * 1000).toLocaleTimeString();
                row.querySelector(".max-temp").innerHTML = Math.round(data.main.temp_max ) + "°C";
                row.querySelector(".min-temp").innerHTML = Math.round(data.main.temp_min - 4) + "°C";
            }
        } catch (e) {
